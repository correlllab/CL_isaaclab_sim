...
NvjpegEncoder::NvjpegEncoder(StreamType type, std::vector<std::string> names) {

  nvjpegCreateSimple(&mNvHandle);

  for (auto& name : names) {
    nvjpegEncoderState_t state;
    nvjpegEncoderStateCreate(mNvHandle, &state, mCuda);
    mStates[name] = state
...
void NvjpegEncoder::EncodeImageBuffer(Image img, StreamType optype, std::vector<unsigned char>& buffer) {
  nvjpegEncoderState_t state = mStates[name];
  unsigned char* dataPtr = reinterpret_cast<unsigned char*>(img.dataPtr);
  nvjpegImage_t nvImg;
  nvImg.channel[0] = dataPtr;

  if (optype == StreamType::RGB) {}
